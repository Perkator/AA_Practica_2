...
class Node():
    ...
    def state(self):
        ...
        self.server.state = config.wanted
        want = Message(msg_type = "accessSc",
                        src = self.id,
                        data = "Hola, this is Node_%i _ counter:%i"%(self.id,self.wakeupcounter))
        self.client.multicast(want, self.collegues)
        while(self.server.replied != len(self.collegues)):
            pass
        self.server.state = config.held
        #SC
        print("This is node %i at the SC", self.id)
        time.sleep(0.001)
        #Exit SC
        self.server.state = config.released
        release = Message(msg_type = "release",
                        src=self.id,
                        data = "Hola, this is Node_%i _ counter:%i"%(self.id,self.wakeupcounter))
        self.client.multicast(release, self.collegues)
